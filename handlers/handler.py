...
def register_handlers_client(dp: Dispatcher):
    dp.register_message_handler(start_command, commands=[
                                'start', 'help'], state="*")
    dp.register_message_handler(info_command, commands=[
                                'info', 'help'], state="*")
    dp.register_message_handler(begin_test_command, commands=[
                                'begin_test'], state="*")
    dp.register_message_handler(last_command, commands=[
                                'Вернуть_вопрос'], state="*")
    dp.register_message_handler(safe_command, commands=[
                                'Сохранить_ответы'], state="*")
    dp.register_message_handler(state_answer_1, Text(
        equals=['A', 'B']), state=StateAnswer.ANS_1)
    dp.register_message_handler(state_answer_2, Text(
        equals=['A', 'B']), state=StateAnswer.ANS_2)
